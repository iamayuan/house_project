<template>
  <div class="b_f5">
    ...
    <div>
      <banner :data="itemdata" :relatedid="promotionHouseId" :relatedflag="4" v-if="itemdata"></banner>
      <div class="midwarp">
        <p class="now_add">
          <span class="lq_now_add_ico">
          <img src="../assets/images/icon8.png"  alt="">
          </span>
          <span class="lq_now_span">首页</span> &gt; <span class="lq_now_span">特惠房</span> &gt;<span class="lq_now_span">{{itemdata.buildingName}}</span>
        </p>
        <ul class="tab_ul">
          <li v-for="(item,index) in infor" @click="qwer=index" :class='{selected:index==qwer}'><a  v-bind:href="item.id">{{item.name}}</a></li>
        </ul>
      ...
      <section id="floor1" class="midwarp">
        ...
        <div class="hauto">
          ...
          <div class="right_detail">
            ...
            <ul class="lp_ul">
              <li>类别：<em>{{itemdata.houseType}}</em></li>
              <li>建筑面积：<em>{{itemdata.houseArea}}</em></li>
              <li>有无电梯：<em>{{itemdata.liftType}}</em></li>
              <li>卧室：<em>{{itemdata.roomType}}</em></li>
              <li>室内面积：<em>{{itemdata.roomArea}}</em></li>
              <li>朝向：<em>{{itemdata.direction}}</em></li>
              <li>客厅：<em>{{itemdata.hallType}}</em></li>
              <li>楼层：<em>{{itemdata.floorNum}}</em></li>
              <li><em></em></li>
              <li>厨房：<em>{{itemdata.cookroomType}}</em></li>
              <li>均价：<em>{{itemdata.averagePrice}}</em></li>
              <li><em></em></li>
              <li>厕所：<em>{{itemdata.washroomType}}</em></li>
              <li>总价：<em>{{itemdata.houseMoney}}</em></li>
              <li><em></em></li>
              <li>阳台：<em>{{itemdata.shineroomType}}</em></li>
              <li>装修标准：<em>{{itemdata.designStandard}}</em></li>
              <li><em></em></li>
              <li>花园：<em>{{itemdata.gardenType}}</em></li>
              <li>产权年限：<em>{{itemdata.ownYear}}</em></li>
            </ul>
          </div>
        </div>
      </section>
      ...
      <div class="tip-box midwarp">
          <p class="tip"><i class="tip-img"></i>免责声明：楼盘信息由开发商提供，最终以政府部门登记备案为准，请谨慎核查。如楼盘信息有误，您可拨打电话投诉：000000000</p>
      </div>
    </div>
    <!-- 预约看房弹框 -->
...
export default {
  ...
  created: function () {
    this.promotionHouseId = this.$utils.getUrlKey('promotionHouseId');
    this.queryPromotionHouseInfo();
  },
  methods: {
    queryPromotionHouseInfo () {
      console.log('------------------location')
      console.log(this.$location)
      this.$http.get(this.$location + '/api/promotionHouse/queryPromotionHouseInfo.do?from=1&timestamp=2&promotionHouseId='+this.promotionHouseId, {}, {
        headers: {
        },
        emulateJSON: true
      }).then((res) => {
        ...
        if (data.errorCode == 0) {
          ...
          let len1 = this.itemdata.demoPicUrl.split(',').length;
          let data1 = this.itemdata.demoPicUrl.split(',');
          let len2 = this.itemdata.housePicUrl.split(',').length;
          let data2 = this.itemdata.housePicUrl.split(',');
          let len3 = this.itemdata.realPicUrl.split(',').length;
          let data3 = this.itemdata.realPicUrl.split(',');
          let len4 = this.itemdata.virtualPicUrl.split(',').length;
    ...
    saveUserPreorder () {
      ...
      this.$http.get(this.$location + '/api/preorder/saveUserPreorder.do?from=1&timestamp=2&relatedFlag=0&token='+ token +'&relatedId='+ this.leaseHouseId +'&realname='+ this.appointmentInfo.name +'&tel='+ this.appointmentInfo.phone +'&preorderTime='+ this.appointmentInfo.preorderTime.toLocaleDateString() +'&message = '+ this.appointmentInfo.message, {}, {
        headers: {
        },
        emulateJSON: true
      }).then((res) => {
    ...
    saveUserRecommendCommission () {
      ...
      this.$http.get(this.$location + '/api/recommendCommission/saveUserRecommendCommission.do?from=1&timestamp=2&token='+ token +'&realname='+ this.appointmentInfo.name +'&tel='+ this.appointmentInfo.phone +'&sex='+ this.appointmentInfo.sex +'&age='+ this.appointmentInfo.age +'&relatedId='+ this.leaseHouseId +'&relatedFlag=0', {}, {
        headers: {
        },
        emulateJSON: true
      }).then((res) => {
