<template>
  <div class="b_f5">
    ...
    <div>
      <banner :data="infodata" :relatedid="secondHouseId" :relatedflag="2" v-if="infodata"></banner>
      <div class='midwarp'>
        <p class="now_add">
          <img src="../assets/images/icon8.png" class="lq_now_add_ico" alt="">
          <span class="lq_now_span">首页</span> &gt;
          <span class="lq_now_span">二手房</span> &gt;
          <span class="lq_now_span">{{infodata.buildingName}}</span>
        </p>
        <ul class="tab_ul">
...
export default {
    ...
    created: function() {
        this.secondHouseId = this.$utils.getUrlKey("secondHouseId");
        this.querySecondHouseInfo();
        this.querySecondSubdistrictInfo();
    },
    methods: {
        querySecondHouseInfo() {
            this.$http
                .get(
                    this.$location +
                        "/api/secondHouse/querySecondHouseInfo.do?from=1&timestamp=2&secondHouseId=" +
                        this.secondHouseId,
                    {},
                    {
                        headers: {},
        ...
        querySecondSubdistrictInfo() {
            this.$http
                .get(
                    this.$location +
                        "/api/secondSubdistrict/querySecondSubdistrictInfo.do?from=1&timestamp=2&secondHouseId=" +
                        this.secondHouseId,
                    {},
                    {
                        headers: {},
        ...
        saveUserPreorder() {
            ...
            this.$http
                .get(
                    this.$location +
                        "/api/preorder/saveUserPreorder.do?from=1&timestamp=2&relatedFlag=2&token=" +
                        token +
                        "&relatedId=" +
                        this.leaseHouseId +
                        "&realname=" +
